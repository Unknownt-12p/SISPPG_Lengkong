...
<head>
    <meta charset="UTF-8">
    <title>Laporan Penyaluran Makanan Bergizi</title>
    <style>
        body {
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th style="width: 15%;">Kode Penyaluran</th>
                <th style="width: 15%;">Kode Pengajuan</th>
                <th style="width: 25%;">Nama Instansi</th>
                <th style="width: 20%;">Menu Makanan</th>
                <th style="width: 10%; text-align: center;">Porsi Salur</th>
                <th style="width: 10%;">Tanggal Salur</th>
            </tr>
        </thead>
        <tbody>
            @php $totalPorsi = 0; @endphp
            @forelse($data as $key => $p)
                @php $totalPorsi += $p->jumlah_disalurkan; @endphp
                <tr>
                    <td class="text-center">{{ $key + 1 }}</td>
                    <td class="fw-bold">{{ $p->kode_penyaluran }}</td>
                    <td>{{ $p->pengajuan->kode_pengajuan }}</td>
                    <td>{{ $p->pengajuan->instansi->nama_instansi }}</td>
                    <td>{{ $p->menu_makanan->nama_menu }}</td>
                    <td class="text-center fw-bold" style="color: #2c7c8f;">{{ number_format($p->jumlah_disalurkan) }}</td>
                    <td>{{ date('d-m-Y', strtotime($p->tanggal_penyaluran)) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" class="text-center" style="padding: 20px; color: #666;">Tidak ada data penyaluran yang sesuai filter.</td>
                </tr>
            @endforelse
            
            @if(count($data) > 0)
                <tr style="background-color: #eee; font-weight: bold;">
                    <td colspan="5" class="text-right">TOTAL PORSI DISALURKAN:</td>
                    <td class="text-center" style="color: #2c7c8f;">{{ number_format($totalPorsi) }}</td>
                    <td></td>
                </tr>
            @endif
        </tbody>
